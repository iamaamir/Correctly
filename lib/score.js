...
// Maps internal suppression issue keys to user-facing hidden-change reasons.
// Iterated in insertion order so multi-reason strings keep a stable order.
const ISSUE_TO_HIDDEN_REASON = {
  "phrase not found in source": "original not found in source text",
  "original phrase is ambiguous": "ambiguous match in source",
  "whole-text change": "whole-text change not shown as individual suggestion",
  "original equals replacement": "original equals replacement",
  "replacement looks like a rewrite": "replacement is a broad rewrite",
  "overlaps earlier change": "overlaps another change",
  "missing original": "change has no original text",
};
...
export function extractDisplayChanges(parsed, originalText) {
  ...
  for (let i = 0; i < parsed.changes.length; i++) {
    ...
    if (entry.suppress || entry.entryScore < MIN_ENTRY_SCORE) {
      ...
      for (const [issue, reason] of Object.entries(ISSUE_TO_HIDDEN_REASON)) {
        if (entry.issues.includes(issue)) reasons.push(reason);
      }
      hiddenChanges.push({
        ...cleanChange,
