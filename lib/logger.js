...
export function createLogger(tag) {
  ...
  return {
    debug: (...args) => {
      if (shouldLog("debug")) console.debug(`%c${tagStr}`, LEVELS.debug.style, ...args);
    },
    info: (...args) => {
      if (shouldLog("info")) console.info(`%c${tagStr}`, LEVELS.info.style, ...args);
    },
    warn: (...args) => {
      if (shouldLog("warn")) console.warn(`%c${tagStr}`, LEVELS.warn.style, ...args);
    },
    error: (...args) => {
      if (shouldLog("error")) console.error(`%c${tagStr}`, LEVELS.error.style, ...args);
    },
